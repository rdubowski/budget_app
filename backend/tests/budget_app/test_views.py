...
def test_retrieve_accounts(api_client, create_logged_user):
    ...
    account1 = Account.objects.create(
        user=user, name="account1", description="Random description1"
    )
    account2 = account = Account.objects.create(
        user=user, name="account2", description="Random des2"
    )
    resp = api_client.get(ACCOUNTS_URL)
...
def test_account_limited_to_user(api_client, create_logged_user):
    ...
    account_for_logged = Account.objects.create(
        user=user_logged, name="account2", description="Randomdes2"
    )
    resp = api_client.get(ACCOUNTS_URL)
    assert resp.status_code == 200
...
def test_create_account_successful(api_client, create_logged_user):
    ...
    exists = Account.objects.filter(user=user, name=payload["name"]).exists()
    assert exists
...
def test_retrieve_account(api_client, create_logged_user):
    payload = {"name": "account1", "description": "Random description1"}
    api_client.post(ACCOUNTS_URL, payload)
    account_id = Account.objects.get(name=payload["name"]).id
...
def test_partial_update_account(api_client, create_logged_user):
    ...
    account = Account.objects.create(
        user=user, name="account1", description="Random description"
    )
...
def test_full_update_account(api_client, create_logged_user):
    ...
    payload = {"name": "accountUpdated", "description": "descriptionupdated"}
    single_account_url = reverse("budget:account-detail", args=(account.id,))
...
def test_list_transactions_for_single_account(
    api_client, create_logged_user, create_account, create_custom_transaction
):
    ...
    serializer_1 = TransactionSerializer(transaction_1, many=False)
    serializer_2 = TransactionSerializer(transaction_2, many=False)
    list_transactions_url = reverse("budget:transactions", args=(account.id,))
    resp = api_client.get(list_transactions_url)
    assert resp.status_code == 200
    assert serializer_1.data in resp.data
    assert serializer_2.data in resp.data


def test_list_transactions_seperate_accounts(
    api_client, create_logged_user, create_account, create_custom_transaction
):
    """Show list of transactions for logged user and his account """
    user = create_logged_user
    ...
    account_2 = Account.objects.create(
        user=user, name="account_2", description="Random description"
    )
    transaction_payload = {
        "amount": 1500,
        "name": "transaction1",
...
def test_create_transaction_successful(
    api_client, create_logged_user, create_account
):
    user = create_logged_user
    account = create_account
    list_transactions_url = reverse("budget:transactions", args=(account.id,))

    payload = {"amount": 5, "name": "fds", "transaction_type": "W"}
    resp = api_client.post(list_transactions_url, data=payload, format="json")
...
def test_delete_transaction_successful(
    api_client, create_logged_user, create_account, create_custom_transaction
):
    user = create_logged_user
    account = create_account
    ...
    transaction = create_custom_transaction(**transaction_payload)
    delete_transaction_url = reverse(
        "budget:transaction_delete", args=(transaction.id,)
    )
    resp = api_client.delete(delete_transaction_url, {})
    assert resp.status_code == 204
