...
def test_create_account(create_user):
    ...
    assert (
        Account.objects.get(name="account1").description
        == "Random description"
    )
...
def test_create_transatction(create_user):
    ...
    transaction = Transaction.objects.create(
        amount=500, name="transaction1", transaction_type="D", account=account
    )
    assert Transaction.objects.all().count() == 1
    assert Transaction.objects.filter(name="transaction1").exists()
    assert (
        str(Transaction.objects.get(name="transaction1"))
        == "transaction1-Deposit"
    )
    assert Transaction.objects.get(name="transaction1").transaction_type == "D"


def test_account_sum_functions_pos_and_neg(
    create_user, create_custom_transaction
):
    user = create_user
    account = Account.objects.create(
        user=user, name="account1", description="Random description"
    ...
    transaction_payload_3 = {
        "amount": 400,
        "name": "transaction3",
        "transaction_type": "W",
        "account": account,
    }
    transaction_payload_4 = {
        "amount": 300,
        "name": "transaction4",
        "transaction_type": "W",
        "account": account,
...
def test_account_sum_functions_negative_only(
    create_user, create_custom_transaction
):
    user = create_user
    account = Account.objects.create(
...
def test_account_sum_functions_positive_only(
    create_user, create_custom_transaction
):
    user = create_user
    account = Account.objects.create(
