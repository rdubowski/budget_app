...
class Account(models.Model):
    ...
    init_balance = models.DecimalField(max_digits=15, decimal_places=2, default=0, blank=True, null=True)
    ...
    def deposit_sum(self):
        deposit_sum = Transaction.objects.filter(account=self, transaction_type='D'
        ).aggregate(sum=Sum('amount'))
        return deposit_sum['sum'] if deposit_sum['sum'] else 0
       

    def withdraw_sum(self):
        withdraw_sum = Transaction.objects.filter(account=self, transaction_type='W'
        ).aggregate(sum=Sum('amount'))
